...
if Config.TG_BOT_USER_NAME_BF_HER is not None and tgbot is not None:
        ...
        if query.startswith("**HyperUserBot-X") and event.query.user_id == bot.uid:
            buttons = [
                (
                    ...
                    Button.url("Repo", "https://github.com/ahirearyan2/HyperUserBot-X"),
                )
            ]
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid:  # pylint:disable=E0602
            ...
            reply_pop_up_alert = "Please get your own HyperUserBot-X, and don't use mine! Join @HyperUserBotXSupport help"
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid:
            ...
            reply_pop_up_alert += (
                "Use .unload {} to remove this plugin © HyperUserBot-X".format(
                    plugin_name
                )
            )
            try:
